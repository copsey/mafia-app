...
const mafia::Rulebook & mafia::Setup_screen::rulebook() const {
   return _rulebook;
}

std::vector<std::string> mafia::Setup_screen::player_names() const {
   return {_player_names.begin(), _player_names.end()};
}

std::vector<mafia::Role::ID> mafia::Setup_screen::rolecard_ids() const {
...
std::vector<mafia::Wildcard::ID> mafia::Setup_screen::wildcard_ids() const {
...
bool mafia::Setup_screen::has_player(const std::string &name) const {
...
bool mafia::Setup_screen::has_rolecard(const std::string &alias) const {
   Role::ID id = _rulebook.get_role(alias).id();
   return _role_ids.count(id) != 0 && _role_ids.at(id) != 0;
}

bool mafia::Setup_screen::has_wildcard(const std::string &alias) const {
   Wildcard::ID id = _rulebook.get_wildcard(alias).id();
   return _wildcard_ids.count(id) != 0 && _wildcard_ids.at(id) != 0;
}

std::size_t mafia::Setup_screen::num_players() const {
   return _player_names.size();
}

std::size_t mafia::Setup_screen::num_rolecards() const {
   std::size_t n{0};
   for (auto &p: _role_ids) n += p.second;
   return n;
}

std::size_t mafia::Setup_screen::num_wildcards() const {
   std::size_t n{0};
   for (auto &p: _wildcard_ids) n += p.second;
   return n;
}

std::size_t mafia::Setup_screen::num_cards() const {
   return num_rolecards() + num_wildcards();
}

void mafia::Setup_screen::add_player(const std::string &name) {
...
void mafia::Setup_screen::add_rolecard(const std::string &alias) {
   const Role &r = _rulebook.get_role(alias);
   ++_role_ids[r.id()];
}

void mafia::Setup_screen::add_wildcard(const std::string &alias) {
   const Wildcard &w = _rulebook.get_wildcard(alias);
   ++_wildcard_ids[w.id()];
}

void mafia::Setup_screen::remove_player(const std::string &name) {
...
void mafia::Setup_screen::remove_rolecard(const std::string &alias) {
   const Role &r = _rulebook.get_role(alias);
   if (_role_ids[r.id()] == 0) {
...
void mafia::Setup_screen::remove_wildcard(const std::string &alias) {
   const Wildcard &w = _rulebook.get_wildcard(alias);
...
void mafia::Setup_screen::clear_all_players() {
   _player_names.clear();
}

void mafia::Setup_screen::clear_rolecards(const std::string &alias) {
   const Role &r = _rulebook.get_role(alias);
   _role_ids[r.id()] = 0;
}

void mafia::Setup_screen::clear_all_rolecards() {
   _role_ids.clear();
}

void mafia::Setup_screen::clear_wildcards(const std::string &alias) {
   const Wildcard &w = _rulebook.get_wildcard(alias);
   _wildcard_ids[w.id()] = 0;
}

void mafia::Setup_screen::clear_all_wildcards() {
   _wildcard_ids.clear();
}

void mafia::Setup_screen::clear_all_cards() {
   clear_all_rolecards();
   clear_all_wildcards();
}

void mafia::Setup_screen::clear_all() {
   clear_all_players();
   clear_all_cards();
}

std::unique_ptr<mafia::Game_log> mafia::Setup_screen::new_game_log() const {
   return std::make_unique<Game_log>(player_names(), rolecard_ids(), wildcard_ids(), _rulebook);
}

void mafia::Setup_screen::do_commands(const std::vector<std::string> &commands) {
...
void mafia::Setup_screen::write(std::ostream &os) const {
...
void mafia::Setup_screen::write_players_list(std::ostream &os) const {
...
void mafia::Setup_screen::write_cards_list(std::ostream &os) const {
   bool write_nl{false};
